...
function OtpInput({ value, onChange, hasError }: OtpInputProps) {
  ...
  return (
    <div className="otp-boxes" role="group" aria-label="Dogrulama kodu">
...
function OtpPageInner() {
  ...
  const verify = useCallback(async (otpCode: string) => {
    ...
    try {
      ...
      if (json.access_token && json.refresh_token) {
        // Supabase session'i cookie'ye yaz
        const supabase = getSupabaseBrowserClient()
        const { error: sessionError } = await supabase.auth.setSession({
          access_token: json.access_token,
          refresh_token: json.refresh_token,
        })
        console.log('[otp] setSession error:', sessionError)

        // Cookie'lerin yazilmasi icin bekle
  ...
  return (
    <div className="auth-card otp-card" data-loading={loading}>
      <button className="otp-back" onClick={() => router.back()} aria-label="Geri git">
        ← Geri
      </button>
      ...
      <div className="auth-card-head">
        ...
        <p className="auth-subtitle">
          <strong>+90 {displayPhone}</strong> numarasina 6 haneli dogrulama kodu gonderdik.
        </p>
      </div>
      ...
      <div className="resend-section">
        {remaining > 0 ? (
          <p className="resend-countdown">
            Kod gelmedi mi? <strong>{remaining}s</strong> sonra tekrar gonderebilirsiniz.
          </p>
        ) : (
      ...
      <style>{`
        ...
        .otp-back {
          ...
          color: var(--gray); cursor: pointer; padding: 4px 0;
        ...
        .otp-loading {
          ...
          gap: 8px; font-size: 13px; color: var(--orange); font-weight: 600;
        }
        ...
        .auth-card {
          ...
          box-shadow: 0 4px 24px rgba(74,44,14,0.08), 0 1px 4px rgba(74,44,14,0.06);
          border: 1px solid rgba(232,224,212,0.8); transition: opacity 0.2s;
        }
        .auth-card[data-loading="true"] { opacity: 0.85; }
        .auth-step-badge {
          display: inline-block; font-size: 10px; font-weight: 700;
          letter-spacing: 2px; text-transform: uppercase;
          color: var(--orange); background: #FEF3EC;
          padding: 4px 10px; border-radius: 20px; margin-bottom: 14px;
        }
        .auth-title {
          font-family: 'Playfair Display', serif; font-size: 36px;
          font-weight: 900; color: var(--brown); line-height: 1.1; margin: 0 0 10px;
        }
...
export default function OtpPage() {
  return (
    <Suspense fallback={<div className="auth-card" style={{ padding: 40 }}>Yukleniyor…</div>}>
      <OtpPageInner />
    </Suspense>
  )
}
...
export default async function BlogPage() {
  ...
  const yazilar = posts ?? []
  const categories = ['Tümü', ...Array.from(new Set(yazilar.flatMap(p => p.tags ?? [])))]
          ...
          <>
            ...
            <Link href={`/blog/${yazilar[0].slug}`} style={{ textDecoration: 'none' }}>
              <div style={{ background: 'white', borderRadius: 20, overflow: 'hidden', boxShadow: '0 2px 16px rgba(74,44,14,0.08)', border: '1px solid rgba(232,224,212,0.4)', marginBottom: 24, display: 'flex', flexWrap: 'wrap' }}>
                <div style={{ minWidth: 200, flex: '1 1 200px', minHeight: 200, background: yazilar[0].cover_image ? 'transparent' : CATEGORY_COLORS[0], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 80, overflow: 'hidden' }}>
                  {yazilar[0].cover_image
                    ? <img src={yazilar[0].cover_image} alt={yazilar[0].title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                    : '📝'
                  }
                </div>
                <div style={{ flex: '2 1 300px', padding: '28px 32px' }}>
                  <div style={{ display: 'flex', gap: 8, marginBottom: 12 }}>
                    ...
                    {(yazilar[0].tags ?? []).slice(0, 1).map(t => (
              ...
              {yazilar.slice(1).map((post, i) => (
                <Link key={post.id} href={`/blog/${post.slug}`} style={{ textDecoration: 'none' }}>
                  <div style={{ background: 'white', borderRadius: 16, overflow: 'hidden', boxShadow: '0 2px 12px rgba(74,44,14,0.07)', border: '1px solid rgba(232,224,212,0.4)', height: '100%', display: 'flex', flexDirection: 'column' }}>
                    <div style={{ height: 120, background: post.cover_image ? 'transparent' : CATEGORY_COLORS[i % CATEGORY_COLORS.length], display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 52, overflow: 'hidden' }}>
                      {post.cover_image

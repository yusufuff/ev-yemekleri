    ...
    <div style={{ minHeight: '100vh', background: '#FAF6EF', fontFamily: "'DM Sans', sans-serif" }}>
      <nav style={{ background: '#4A2C0E', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          {NAV_LINKS.map(([l, h]) => (
            <Link key={h} href={h} style={{ color: h === '/admin/siparisler' ? 'white' : 'rgba(255,255,255,0.7)', fontSize: 12, textDecoration: 'none', fontWeight: h === '/admin/siparisler' ? 700 : 500 }}>{l}</Link>
          ))}
        </div>
      </nav>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
          <div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
              ...
              <p style={{ color: '#3D6B47', fontSize: 13, margin: 0, fontWeight: 600 }}>Gerçek zamanlı izleniyor</p>
              {yeniSiparis && (
                <span style={{ background: '#E8622A', color: 'white', fontSize: 11, fontWeight: 700, padding: '2px 10px', borderRadius: 20 }}>🔔 Yeni sipariş!</span>
              )}
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {['all', 'pending', 'preparing', 'on_the_way', 'delivered', 'cancelled'].map(s => (
              <button key={s} onClick={() => setFilter(s)} style={{
                ...
                background: filter === s ? '#E8622A' : 'white',
                color: filter === s ? 'white' : '#8A7B6B',

...
export default function YemekFotolarPage() {
  ...
  useEffect(() => { kayitlariYukle() }, [])

  useEffect(() => {
    if (aramaMetni.trim() === '') {
      setFiltreli(kayitlar)
    } else {
      setFiltreli(kayitlar.filter(k => k.food_name.toLowerCase().includes(aramaMetni.toLowerCase())))
    }
  }, [aramaMetni, kayitlar])
  ...
  const dosyaSec = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + fotolar.length > 20) {
      setMesaj({ tip: 'hata', metin: 'En fazla 20 fotoğraf ekleyebilirsiniz.' })
      return
    }
    setFotolar(prev => [...prev, ...files])
    const urls = files.map(f => URL.createObjectURL(f))
    setOnizleme(prev => [...prev, ...urls])
  }
    ...
    try {
      ...
      for (const foto of fotolar) {
        ...
        const { data: uploadData, error: uploadErr } = await supabase.storage
          .from('food-photos')
          .upload(dosyaAdi, foto, { contentType: foto.type, upsert: false })
  ...
  const sil = async (id: string, photoUrl: string) => {
    ...
    try {
      ...
      setSilYukleniyor(null)
    }
  }

  // Yemek adına göre grupla
  const gruplar: Record<string, any[]> = {}
  filtreli.forEach(k => {
    if (!gruplar[k.food_name]) gruplar[k.food_name] = []
    gruplar[k.food_name].push(k)
  })
      ...
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'28px 24px' }}>
        ...
        <div style={{ background:'white', borderRadius:16, padding:24, marginBottom:24, boxShadow:'0 2px 12px rgba(74,44,14,0.08)' }}>
          ...
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr auto', gap:12, marginBottom:16 }}>
            <div>
              ...
              <input
                value={yemekAdı}
                onChange={e => setYemekAdı(e.target.value)}
                placeholder="Örn: Sütlaç, Mercimek Çorbası..."
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:10, marginBottom:16 }}>
              {onizleme.map((url, i) => (
                <div key={i} style={{ position:'relative' }}>
                  ...
                  <button
                    onClick={() => fotoCikar(i)}
                    style={{ position:'absolute', top:-6, right:-6, width:20, height:20, borderRadius:'50%', background:'#ef4444', color:'white', border:'none', cursor:'pointer', fontSize:12, fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center' }}
                  >×</button>
                </div>
              ))}
            </div>
          ...
          <div style={{ display:'flex', gap:12, alignItems:'center' }}>
            ...
            <span style={{ fontSize:12, color:'#888' }}>En az 5, en fazla 20 fotoğraf önerilir</span>
          </div>
        </div>
        ...
        <div style={{ background:'white', borderRadius:16, padding:20, marginBottom:24, boxShadow:'0 2px 12px rgba(74,44,14,0.08)' }}>
          ...
          <div style={{ marginTop:8, fontSize:12, color:'#888' }}>
            {Object.keys(gruplar).length} yemek, {filtreli.length} fotoğraf
          </div>
        </div>
            ...
            <div style={{ display:'flex', flexWrap:'wrap', gap:8 }}>
              {fotograflar.map((f: any) => (
                <div key={f.id} style={{ position:'relative' }}>
                  ...
                  <button
                    onClick={() => sil(f.id, f.photo_url)}
                    disabled={silYukleniyor === f.id}

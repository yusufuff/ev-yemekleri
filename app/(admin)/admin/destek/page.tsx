    ...
    <div style={{ minHeight: '100vh', background: '#FAF6EF', fontFamily: "'DM Sans', sans-serif" }}>
      <nav style={{ background: '#4A2C0E', padding: '0 24px', height: 56, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 14, flexWrap: 'wrap' }}>
          {NAV_LINKS.map(([l, h]) => (
            <Link key={h} href={h} style={{ color: h === '/admin/destek' ? 'white' : 'rgba(255,255,255,0.7)', fontSize: 12, textDecoration: 'none', fontWeight: h === '/admin/destek' ? 700 : 500 }}>{l}</Link>
          ))}
        </div>
      </nav>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: secili ? '1fr 1fr' : '1fr', gap: 20 }}>
          ...
          <div style={{ background: 'white', borderRadius: 16, boxShadow: '0 2px 12px rgba(74,44,14,0.08)', overflow: 'hidden' }}>
            {loading ? (
              ...
              <div style={{ padding: 40, textAlign: 'center', color: '#8A7B6B' }}>Talep bulunamadi</div>
            ) : talepler.map(t => {
            ...
            <div style={{ background: 'white', borderRadius: 16, boxShadow: '0 2px 12px rgba(74,44,14,0.08)', padding: 24 }}>
              ...
              <div style={{ marginBottom: 12 }}>
                ...
                <textarea
                  ...
                  rows={4}
                  style={{ width: '100%', padding: 12, borderRadius: 10, border: '1.5px solid #E8E0D4', fontSize: 14, fontFamily: 'inherit', resize: 'vertical', boxSizing: 'border-box' as const }}

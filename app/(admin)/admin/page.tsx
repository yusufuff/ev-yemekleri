...
export default function AdminDashboard() {
  ...
  const loadData = async () => {
    try {
      ...
      const [ordersRes, chefsRes, stockRes] = await Promise.all([
        supabase.from('orders').select('id, order_number, status, total_amount, subtotal, created_at').order('created_at', { ascending: false }).limit(50),
        supabase.from('chef_profiles').select('id, is_open, verification_status, pending_approval').order('created_at', { ascending: false }),
        supabase.from('menu_items').select('id, name, remaining_stock').eq('is_active', true).lt('remaining_stock', 3),
      ])
      ...
      const pendingChefs = (chefsRes.data ?? []).filter(c => c.pending_approval || c.verification_status === 'pending').length
      setOrders(allOrders)
      setChefs(chefsRes.data ?? [])
      setStock(stockRes.data ?? [])
      setStats({ totalRevenue: Math.round(totalRevenue), commission: Math.round(totalRevenue * 0.1), orderCount: todayOrders.length, pendingChefs })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <div style={{ padding: '28px 24px', fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <h1 style={{ fontFamily: "'Playfair Display',serif", fontSize: 24, fontWeight: 900, color: '#4A2C0E', margin: '0 0 24px' }}>Dashboard</h1>
      ...
      {lateOrders.length > 0 && (
        <div style={{ background: '#FEE2E2', border: '2px solid #DC2626', borderRadius: 12, padding: '12px 16px', marginBottom: 20, display: 'flex', alignItems: 'center', gap: 10 }}>
          <span>🚨</span>
          <span style={{ color: '#DC2626', fontWeight: 700, fontSize: 14 }}>{lateOrders.length} sipariş 30 dakikayı geçti!</span>
        </div>
      )}
        ...
        <div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 24 }}>
            ...
            {activeOrders.map(order => {
              ...
              return (
                <div key={order.id} style={{ background: 'white', borderRadius: 12, padding: '12px 16px', border: isLate ? '2px solid #DC2626' : '1px solid #E8E0D4', display: 'flex', alignItems: 'center', gap: 12 }}>
